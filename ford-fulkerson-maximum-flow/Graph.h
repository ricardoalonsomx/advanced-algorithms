...
public:
    ...
    void addEdge(int firstNode, int secondNode, int weight) { this->addEdge(*new Edge(firstNode, secondNode, weight)); };
    void addEdgePrim(int firstNode, int secondNode, int weight, int unreached) { this->addEdgePrim(*new Edge(firstNode, secondNode, weight), unreached); };
    ...
    void changeWeightAt(int index, int weight) { edges.at(index).setWeight(weight); };
    ...
    int findEdgeIndex(int firstNode, int secondNode);
...
Graph::Graph(int number_of_edges)
{
    edges.assign(number_of_edges, *new Edge);
}
...
void Graph::printEdges()
{
    for (int i = 0; i < edges.size(); i++) {
        std::cout << edges.at(i).getFirstNode() << ' ' << edges.at(i).getSecondNode() << ' ' << edges.at(i).getWeight();
        
        if (edges.at(i).isResidual()) {
            std::cout << " t";
        }
...
int Graph::findEdgeIndex(int firstNode, int secondNode)
{
    for (int i = 0; i < edges.size(); i++) {
        if (firstNode == edges.at(i).getFirstNode() && secondNode == edges.at(i).getSecondNode()) {
            return i;
        }
    }
